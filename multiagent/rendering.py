...
# 画扇形
def make_conical(radius=10, angle_cover=2 * math.pi / 3, res=30, filled=True):
    points = [(0, 0)]
    for i in range(res + 1):  # 保证i可以等于res
        ang = (angle_cover * i / res) - angle_cover / 2
        points.append((math.cos(ang) * radius, math.sin(ang) * radius))
    points.append((0, 0))
    if filled:
        return FilledPolygon(points)
...
# 画智能体的朝向
def make_quiver(angle=0, radius=0.1, filled=True):
    points = [(0, 0), (radius * math.cos(angle), radius * math.sin(angle))]
    return PolyLine(points, True)
